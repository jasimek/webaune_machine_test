...
const page = usePage()
const isValidImageUrl = ref(true)
function createName(name) {
    let name_length = name.split(' ').length;
    if (name_length == 1) {
        return name.toUpperCase().slice(0, 2);
    } else {
        return name.toUpperCase().split(' ').map((word, key) => {
            if (key < 2) {
...
function checkImageValid() {
    ...
    if (!page.props.auth.user.image_url || !isValidImageUrl) {
        isValidImageUrl.value = false;
    }
}
...
<template>
    <nav class="fixed z-30 w-full bg-white border-b border-gray-200 not-print">
            ...
            <div class="flex items-center justify-between">
                ...
                <div class="flex items-center">
                    ...
                    <!-- <button type="button" data-dropdown-toggle="notification-dropdown"
                        class="p-2 text-gray-500 rounded-lg hover:text-gray-900 hover:bg-gray-10">
                    ...
                    <div class="flex items-center ml-3">
                        <div>
                            <button type="button"
                                ...
                                <img class="w-8 h-8 rounded-full" v-if="isValidImageUrl"
                                    :src="page.props.auth.user.image_url" alt="user photo" @error="checkImageValid">
                                <div v-if="!isValidImageUrl"
                                    class="relative inline-flex items-center justify-center w-10 h-10 overflow-hidden bg-gray-100 rounded-full dark:bg-gray-600">
                                    <span class="font-medium text-gray-600 dark:text-gray-300">{{
                                        createName(page.props.auth.user.name) }}</span>
                                </div>
                            </button>
                        </div>
