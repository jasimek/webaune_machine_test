...
const props = defineProps(
	{ permissions: Object, role: Object }
);
...
function initCheckBox() {
	let count = 0
	for (const item in props.permissions['permission']) {

		for (const key in props.permissions['permission'][item]) {
			count++
			props.role.permissions.forEach(element => {
				if (element.name == props.permissions['permission'][item][key].name) {
					selected_permissions.value.push(element.name)
				}
			});
		}
		setTimeout(() => {
			var checked_checkbox = document.getElementById('module_' + item).querySelectorAll('input[name=checkbox]:checked')
			if (checked_checkbox.length > 0) {
				if (props.permissions['permission'][item].length == checked_checkbox.length) {
					document.getElementById('permission_' + item).checked = true
				} else {
					document.getElementById('permission_' + item).checked = false
				}
			}

		}, 100);
	}
	if (props.role.permissions.length == count) isCheckAll.value = true
}
...
function update() {
	if (selected_permissions.value.length > 0) {
		...
		form.put(route('user-roles.update', [props.role.id]), {
			preserveScroll: true,
			onSuccess: () => {
			},
			onError: () => {
		...
		<div class="px-6 py-3">
			...
			<div class="bg-white rounded-xl mt-4 min-h-screen">
				<div class="pt-9 pl-[27px] pr-[22px]">
					...
					<Accordion v-for="(permission, key) in permissions['permission']"
						class="mb-3 border-b dark:border-gray-700">
						<accordion-panel>
							<accordion-header>{{ key }}</accordion-header>
							<accordion-content>
								<div class="grid gap-1 grid-cols-4 mt-4" :id="'module_' + key">
									<div class="flex items-center mb-4 col-span-full">
										<input type="checkbox" :id="'permission_' + key"
											@change="selectPermissionAll(key, $event)"
											class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600">
										<label for="default-checkbox"
											class="ml-2 text-sm font-medium text-gray-900 dark:text-gray-300">Select
											All</label>
									</div>
									<div class="flex items-center mb-4" v-for="item in permission">
					...
					<div class="flex justify-center">
						<button type="button" @click="update()" :disabled="form.processing"
